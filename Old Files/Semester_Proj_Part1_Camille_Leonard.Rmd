...
set.seed(4)
glm.fit_c <- caret::train(Blue_Tarp_or_Not~Red+Green+Blue,
                    data = blueTarpTrain,
                    method="glm",
                    family="binomial",
...
```{r}
glm.prob_c <- predict(glm.fit_c, newdata=blueTarpTest, positive = '1', type = "prob") #returns df with col 0 (prob not blue tarp) and 1 (prob blue tarp)
glm.pred_50_c <- ifelse(glm.prob_c[,2]>0.5,1,0)
cm.glm_50_c <- confusionMatrix(factor(glm.pred_50_c), factor(blueTarpTest$Blue_Tarp_or_Not)) 
cm.glm_50_c
```

```{r}
glm.pred_80_c <- ifelse(glm.prob_c[,2]>0.8,1,0)
cm.glm_80_c <- confusionMatrix(factor(glm.pred_80_c), factor(blueTarpTest$Blue_Tarp_or_Not)) 
cm.glm_80_c
```

```{r}
par(pty="s")
glm_50_c <- roc(blueTarpTest$Blue_Tarp_or_Not, glm.pred_50_c, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Positive Percentage", col="#965fd4", lwd=4, print.auc=TRUE, main="GLM ROC Curves") 
glm_80_c <- roc(blueTarpTest$Blue_Tarp_or_Not, glm.pred_80_c, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Positive Percentage", col="blue", lwd=4, print.auc.y=42,print.auc=TRUE, main="GLM ROC Curves", add=TRUE) 
...
par(mfrow=c(1,2))
par(pty="s")
rates_glm_50 <- prediction(glm.pred_50_c, blueTarpTest$Blue_Tarp_or_Not)
roc_glm_50<-performance(rates_glm_50,measure="tpr", x.measure="fpr")
plot(roc_glm_50, colorize = TRUE, main="ROC Curve GLMc - 0.5 ")
lines(x = c(0,1), y = c(0,1), col="red")
auc_glm_50 <- performance(rates_glm_50, measure = "auc")
auc_glm_50@y.values

rates_glm_80 <- prediction(glm.pred_80_c, blueTarpTest$Blue_Tarp_or_Not)
...
qda.class=ifelse(qda.pred[,2]>0.5,1,0)
...
knn.class=ifelse(knn.pred[,2]>0.5,1,0)
...
#### References 
need to add the things I read... anything at all... 